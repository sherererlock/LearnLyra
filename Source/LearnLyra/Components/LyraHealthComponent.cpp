...
void ULyraHealthComponent::InitializeWithAbilitySystem(ULyraAbilitySystemComponent* InASC)
{
	...
	if (!AbilitySystemComponent)
	{
		UE_LOG(LogTemp, Error, TEXT("LyraHealthComponent: Cannot initialize health component for owner [%s] with NULL ability system."), *GetNameSafe(Owner));
		return;
	}
...
void ULyraHealthComponent::OnRep_DeathState(ELyraDeathState OldDeathState)
{
	...
	if (OldDeathState > NewDeathState)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("LyraHealthComponent: Predicted past server death state [%d] -> [%d] for owner [%s]."), (uint8)OldDeathState, (uint8)NewDeathState, *GetNameSafe(GetOwner()));
		return;
	}
	...
	if (OldDeathState == ELyraDeathState::NotDead)
	{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("LyraHealthComponent: Invalid death transition [%d] -> [%d] for owner [%s]."), (uint8)OldDeathState, (uint8)NewDeathState, *GetNameSafe(GetOwner()));
		}
	}
